...
class ImageGd{
     ...
     * @param  integer $locate 文字写入位置 THINKIMAGE_WATER_SOUTHEAST [0,0]
     ...
     * @param  [type]  $locate [定义位置]
     * @param  integer $angle  [旋转角度]
     * @return [type]          [返回画布]
     */
    public function writetext($text, $font, $size, $color = '#00000000', 
        $locate = THINKIMAGE_WATER_SOUTHEAST, $angle = 0){
        //资源检测
        if(empty($this->img)) throw new Exception('没有可以被写入文字的图像资源');
        if(!is_file($font)) throw new Exception("不存在的字体文件：{$font}");
        ...
        do{
            /* 写入文字 */
            $col = imagecolorallocatealpha($this->img, $color[0], $color[1], $color[2], $color[3]);
            imagettftext($this->img, $size, $angle, $x , $y , $col, $font, $text);
        } while(!empty($this->gif) && $this->gifNext());
    }
